...
#         key = (pay_dt.year, pay_dt.month)
#         payment_month_keys.add(key)
#         payment_date_map[key] = f"{last_day:02d}/{pay_dt.month:02d}/{pay_dt.year}"
#         k += 1
...
#     for i in range(total_rows):
#         current  = dt + relativedelta(months=i)
#         mese     = f"{MONTHS_IT[current.month - 1]} {current.year}"
#         key      = (current.year, current.month)
...
# """--------corect claude code but date issue------------------------
# """
# # main.py — PDF → DOCX + Excel Extractor (FastAPI version)
# # FIXED: dynamic extraction, no hardcoded commission rates
# """
...
# def fix_schedule(contract: dict) -> list:
#     """
#     Build the full monthly schedule for one contract.
 
#     Logic (100% from PDF, zero hardcoded assumptions):
#     --------------------------------------------------
#     1. contract_date  = "data_contratto" field (DD/MM/YYYY)
#     2. start_date     = contract_date + 40 days
#                         (PDF: "Il Contratto decorre entro il quarantesimo giorno")
#     3. payment_dates  = start_date + k * payment_interval  months,
#                         for k = 1, 2, 3, …  until the contract ends
#                         (PDF Art.4.5: payment on maturazione semestrale/trimestrale)
#     4. All months from contract_date month to end are listed as rows;
#        only the computed payment months get a date and amount.
#     """
 
#     # ── 1. Parse contract date ────────────────────────────────────────────────
#     date_str = contract.get("data_contratto", "")
#     try:
#         contract_date = datetime.strptime(date_str, "%d/%m/%Y")
#     except Exception:
#         contract_date = datetime(2026, 1, 13)   # only if field is genuinely missing
 
#     # ── 2. Compute start date (grace period = 40 days, per PDF) ──────────────
#     # start_date = contract_date + relativedelta(days=40)
#     start_date = contract_date
 
#     # ── 3. Contract parameters (all from extracted PDF data) ─────────────────
#     durata_mesi = int(float(str(contract.get("durata_mesi", "12")).replace(",", ".")))
#     if durata_mesi <= 0:
#         durata_mesi = 12
...
#     # ── 4. Payment interval (months) from PDF field ───────────────────────────
#     if "mens" in frequenza:
#         payment_interval = 1
#     elif "trim" in frequenza:
#         payment_interval = 3
#     elif "ann" in frequenza:
#         payment_interval = 12
#     else:  # default: semestrale
#         payment_interval = 6
 
#     # ── 5. Amount per payment period ──────────────────────────────────────────
#     if pct_periodo == 0 and comm_annuale > 0:
#         pct_periodo = comm_annuale / (12 / payment_interval)
#     amount_per_period = importo * (pct_periodo / 100)
 
#     # ── 6. Build set of payment dates (pure interval math from start_date) ────
#     #   Payment k occurs at:  start_date + k * payment_interval months
#     #   We take the LAST DAY of that month (as per PDF / current behaviour).
#     payment_month_keys = set()   # (year, month) tuples that are payment months
#     payment_date_map   = {}      # (year, month) -> formatted date string
 
#     k = 1
#     while True:
#         pay_dt = start_date + relativedelta(months=k * payment_interval)
#         # Stop when this payment date is beyond the contract duration
#         # Duration measured from start_date
#         if pay_dt > start_date + relativedelta(months=durata_mesi):
#             break
#         last_day = calendar.monthrange(pay_dt.year, pay_dt.month)[1]
#         key = (pay_dt.year, pay_dt.month)
#         payment_month_keys.add(key)
...
#     # ── 7. Build row list — one row per month of the contract ─────────────────
#     #   Rows start from contract_date month (not start_date month).
#     schedule = []
#     row_start = datetime(contract_date.year, contract_date.month, 1)
 
#     for i in range(durata_mesi):
#         current = row_start + relativedelta(months=i)
#         mese = f"{MONTHS_IT[current.month - 1]} {current.year}"
#         key  = (current.year, current.month)
...
#---corectly full dynamically worked pdf with openai-------------------
# """
# main.py — PDF → DOCX + Excel Extractor (FastAPI version)
# FIXED: dynamic extraction, no hardcoded commission rates
# """
...
#         is_payment_month = (months_active > 0) and (months_active % payment_interval == 0)
...
#             amt      = amount_per_period
#         else:
#             pay_date = ""   # empty, no dash
#             amt      = 0.0

#         schedule.append({
#             "mese":                       mese,
#             "data_pagamento_commissioni": pay_date,
#             "management_mensile":         f"{amt:.2f}" if amt else "",  # empty, no dash
#             "_amount":                    amt,
#         })
